...
## 엣지 케이스 스윕 (2026-06-12 도입 — 대표값만 검증하다 새는 버그 방지)
배경 실례: 명세 전수 대조 + E2E PASS 후에도 "날짜 휠을 4개월 돌려 두 자리 월+일
('10월 14일')이 선택돼 18sp Bold로 커지는 순간 두 줄로 줄바꿈"되는 버그가 사람 사용 중
발견됨. 원인 = 시나리오가 대표값(오늘 날짜)만 거침. 아래 스윕으로 유형 자체를 차단한다.

**원칙: 관점은 늘리되 상태당 확인 비용은 고정.** 텍스트 확인(`ui.py texts` ~200토큰) 우선,
스크린샷은 시각으로만 판별되는 항목·Fail 의심 시에만 crop 1장(~0.5k). 화면당 스윕
스크린샷 상한 2장.

### Tier 1 — 극단값 (입력·피커·동적 텍스트가 있는 화면만)
- **피커/휠**: 가장 긴 라벨이 "선택 상태(폰트 확대·볼드)"가 되도록 끝까지 돌린 뒤 확인.
  줄바꿈은 텍스트 덤프로 안 잡히므로 이 항목만 crop 스크린샷 1장으로 시각 확인
- **숫자 입력**: 최대 자릿수(예: 99,999,999원)로 저장 → 홈/상세/통계에서 잘림 없는지 texts로 확인
- **텍스트 입력**: 최장 길이 메모·이름으로 저장 → 리스트 행에서 말줄임 처리 확인
- **리스트**: 빈 상태(0건) / 1건 / 다건 각각 핵심 마커 texts 확인
- 검증 후 만든 극단 데이터는 삭제해 시연/시드 데이터를 오염시키지 않는다

### Tier 2 — 레이아웃 강건성 (신규 화면 1회 + 전역 레이아웃 변경 시)
각 상태에서 핵심 마커 texts 확인, 깨짐 의심 시에만 crop 1장:
- 가로모드: `mobile_set_orientation(landscape)` → 끝나면 portrait 복구
- 좁은 폭 시뮬: `adb shell wm size 760x2640` → 끝나면 `wm size reset` **필수**
- 큰 글꼴: `adb shell settings put system font_scale 1.3` → 끝나면 `1.0` 복구 **필수**

### 복구 체크리스트 (스윕 종료 시 전부 확인)
`wm size reset` / `font_scale 1.0` / orientation portrait / `animator_duration_scale 1`
