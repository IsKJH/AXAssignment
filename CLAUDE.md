...
- 애니메이션: 도넛 스윕(800ms), 카운트업·프로그레스(700ms), 바 성장(700ms),
  말풍선 팝, 스플래시 등장+로딩점 3개, 프로그레스 색 전환
- **리스트 스태거 공용화**: `core/component/StaggeredAppear`(50ms/행 페이드+플로트업,
  진입 120ms 윈도 슬롯 보상 — 스크롤·주기 전환으로 늦게 구성되는 행은 즉시 표시).
  홈·통계·카테고리 리스트 + 빈 상태 뷰 공용
- **화면 전환(M3 모션)**: 푸시/팝 400ms Emphasized Decelerate, 거래 추가 모달
  슬라이드업(450ms)/다운(350ms 가속), 탭 전환 무애니메이션 즉시(대중 패턴), 스플래시 페이드
- 홈 리스트 `animateItem`(삭제 시 빈자리 메꿈), FAB 프레스 스프링 스케일(0.86)+리플
